...
async function loadRecipe(id) {
  ...
  const demoMatch = id === DEMO_RECIPE.id
    ? DEMO_RECIPE
    : DEMO_RELATED.find(r => r.id === id) || { ...DEMO_RECIPE, id };
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:3rem;padding:2rem 0;align-items:start">
      <div>
        <div style="display:flex;gap:8px;margin-bottom:1rem;flex-wrap:wrap">
          ${(recipe.tags||[]).map(t=>`<span style="padding:4px 14px;border-radius:999px;background:var(--green-pale);color:var(--green);font-size:0.72rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase">${t}</span>`).join('')}
        </div>
